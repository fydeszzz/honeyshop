...
import { T, STORE_ITEMS, RARITY, GACHA_TABLE, GACHA_BAGEL_PRICE } from "../constants";
  ...
  return(
    <div style={{animation:"fadeSlide .3s ease"}}>
      <div style={{display:"flex",gap:8,padding:"13px 14px 12px"}}>
        {[["shop","Store"],["gacha",null]].map(([id,label])=>(
...
function ShopBuyView({S,onBuy}){
  ...
  // ── MOCK: preview "owned" and "sold out" card states — TODO: remove before ship
  const MOCK_OWNED   = ["sign_wood"]; // treat this id as already purchased
  const MOCK_SOLDOUT = ["table_b"];   // treat this id as qty = 0
  ...
  return(<>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
      {items.map(item=>{
        const owned=S.owned.includes(item.id)||MOCK_OWNED.includes(item.id);
        const soldOut=MOCK_SOLDOUT.includes(item.id)&&!owned;
        const canBuy=S.currency>=item.price&&!owned&&!soldOut;
        return(
          <div key={item.id} className={owned?`rarity-${item.rarity}`:""} style={{background:RARITY[item.rarity].bg,border:`2.5px solid ${RARITY[item.rarity].border}`,borderRadius:18,padding:"16px 12px 14px",textAlign:"center",position:"relative"}}>
            {/* sold-out overlay */}
            {soldOut&&<div style={{position:"absolute",inset:0,borderRadius:18,background:"rgba(255,255,255,.55)",backdropFilter:"blur(1px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1}}><span style={{fontWeight:900,fontSize:12,color:"#999",background:"#eee",borderRadius:10,padding:"3px 10px",border:"1.5px solid #ccc"}}>Sold Out</span></div>}
            ...
            <div style={{fontWeight:800,fontSize:13,color:T.textDark,marginBottom:10,lineHeight:1.3}}>{item.name}</div>
            <button
              onClick={()=>canBuy&&onBuy(item)}
              disabled={owned||soldOut||S.currency<item.price}
              style={{width:"100%",
                background:owned?"#E0E0E0":soldOut?"#EEEEEE":canBuy?`linear-gradient(135deg,${T.aqua},${T.pink})`:T.snow,
                border:owned||soldOut||canBuy?"none":`2px solid ${T.aquaLight}`,
                borderRadius:12,padding:"7px",
                color:owned?"#9E9E9E":soldOut?"#AAAAAA":canBuy?T.white:T.textLight,
                fontSize:11,fontWeight:800}}>
              {owned?"✅ Owned":soldOut?"— Sold Out —":<>{item.price} <img src="/images/currency/honeypot.png" style={{width:13,height:13,objectFit:"contain",verticalAlign:"middle"}}/></>}
            </button>
          </div>
        );
      })}
